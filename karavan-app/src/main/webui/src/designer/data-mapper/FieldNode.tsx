...
export function FieldNode({ data }: NodeProps<FieldNodeData>) {
    ...
    // Compute depth from JSON Pointer style path (e.g. '/address/street' -> depth 1)
    const slashCount = (field.path.match(/\//g) || []).length;
    const indent = Math.max(0, slashCount - 1);
    ...
    return (
        ...
        >
            {/* Only primitive fields should have connectable ports */}
